...
def start_training(config: dict, endpoint: object) -> None:
    ...
    if os.path.exists(config["train"]["metrics_path"]):
        ...
        old_metrics = {"roc_auc": 0, "precision": 0, "recall": 0, "f1": 0, "logloss": 0, "AMEX": 0}
    ...
    # Вывод метрик
    roc_auc, precision, recall, f1_metric, logloss, amex = st.columns(6)
    roc_auc.metric(
        "ROC-AUC",
        new_metrics["roc_auc"],
    ...
    amex.metric(
        "AMEX",
        new_metrics["AMEX"],
        f"{new_metrics['AMEX'] - old_metrics['AMEX']:.3f}",
    )
